...
export function validateInput(input, validations) {
  ...
  Object.keys(input).forEach((fieldName) => {
    // Validate field
    const validationRlt = validateField(fieldName, input[fieldName], validations);

    // Add errors of current field into formErrors object.
    if (!validationRlt.isValid) {
      inputErrors[fieldName] = validationRlt.errors;
    }
  });

  return {
    isValid: Object.keys(inputErrors).length === 0,
    errors: inputErrors
  };
}
